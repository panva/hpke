  ...
  test.describe(`pummel (${name})`, () => {
          ...
          for (const psk of [undefined, new Uint8Array(32)]) {
            ...
            test.describe(label(suite, mode), () => {
              it('Roundtrip SendExport<>ReceiveExport', async (t: test.TestContext) => {
                ...
                const { encapsulatedSecret: enc, exportedSecret: exported } =
                  await suite.SendExport(pkR, exporterContext, L, { psk, pskId })
    ...
    test.describe('KDF input length limits', () => {
      for (const KDF of KDFS.values()) {
        ...
        // Use first supported KEM and AEAD from the current test round
        const KEM = [...KEMS.values()].find((k) => k.name === 'KEM_DHKEM_P256_HKDF_SHA256')!
        const AEAD = [...AEADS.values()].find((a) => a.name === 'AEAD_AES_128_GCM')!
        const suite = new HPKE.CipherSuite(KEM.factory, KDF.factory, AEAD.factory)
